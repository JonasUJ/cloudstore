...
from ..forms import CloudstorePasswordChangeForm, UserEditForm
...
class AccountView(LoginRequiredMixin, View):
    ...
    def get(self, request: HttpRequest) -> HttpResponse:
        ...
        return render(request, 'account/account.html', {'form_user': form_user,
                                                        'form_password': form_password})

    def post(self, request: HttpRequest) -> HttpResponse:
        form_user = UserEditForm(request.POST, instance=request.user, prefix='form_user')
        form_password = CloudstorePasswordChangeForm(request.user,
                                                     data=request.POST,
                                                     prefix='form_password')
        if any('form_user' in k for k in request.POST):
            form_password = CloudstorePasswordChangeForm(request.user, prefix='form_password')
