...
videotypes = ['MP4', 'WEBM', 'OGV', 'AVI', 'MOV'];
audiotypes = ['MP3', 'OGG', 'WAV'];
displayabletypes = [...imgtypes, ...videotypes, ...audiotypes];

function isAnyType(ext, types) {
    // If the file is any of the file types passed
    if (ext.length > 0) {
        return types.includes(ext.substring(1).toUpperCase());
    }
